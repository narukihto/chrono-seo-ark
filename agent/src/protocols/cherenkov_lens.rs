...
// Shared thread-safe error type for concurrent execution
type ArkError = Box<dyn Error + Send + Sync>;
...
impl CherenkovLens {
    ...
    pub async fn scan(api_key: &str) -> Result<Vec<SeoSignal>, ArkError> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| Box::new(e) as ArkError)?;

        // Unified vector for boxed futures to resolve E0308 type mismatch
        // Note: Using ArkError (Send + Sync) to satisfy Tokio threads
        let mut tasks: Vec<BoxFuture<Result<Vec<SeoSignal>, ArkError>>> = Vec::new();
    ...
    async fn fetch_github_trends(client: &reqwest::Client) -> Result<Vec<SeoSignal>, ArkError> {
    ...
    async fn fetch_global_news(client: &reqwest::Client, key: &str) -> Result<Vec<SeoSignal>, ArkError> {
        let url = format!(
